(function () {
  // -------- CONFIG --------
  const DISPLAY_TICKER = "Reliance";             // Company name to display in tooltip
  const DATA_URL = "../Reliance.json";           // Path to your JSON feed
  const POLL_INTERVAL_MS = 5000;                 // Poll every 5s
  const MAX_POINTS = 200;                         // Show max 40 points
  ...
  function createChartOnCanvas(canvas) {
    ...
    const cfg = {
      ...
      data: {
        labels: [], datasets: [{
          data: [],
          borderColor: "#f59b0bff",
          backgroundColor: "#e0911251",
          borderWidth: 2,
          pointRadius: 2,
          pointHoverRadius: 3,
          pointBackgroundColor: "#fff",
          pointBorderColor: "#f59b0bff",
          pointBorderWidth: 2,
          tension: 0.1,
          fill: true
        }]
  ...
  const arraysEqualNumeric = (a, b) => a.length === b.length && a.every((x, i) => nearlyEqual(Number(x), Number(b[i])));
  function safeChartUpdate(chart) {
    if (!chart) return;
    if (chart.__updating) return;
    chart.__updating = true;
    requestAnimationFrame(() => { try { chart.update("none"); } catch { chart.update(); } chart.__updating = false; });
  }
  ...
  async function start() {
    try {
      ...
      window.addEventListener("beforeunload", () => { if (pollIntervalId) clearInterval(pollIntervalId); if (currentAbort) currentAbort.abort(); });
